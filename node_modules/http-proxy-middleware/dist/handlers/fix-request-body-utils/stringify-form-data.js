...
function getMultipartBoundary(contentType) {
    ...
    if (!boundary || CR_OR_LF.test(boundary)) {
        throw new HttpProxyMiddlewareError('[HPM] invalid multipart boundary detected.', `${ERROR_CODE_PREFIX}_BOUNDARY`);
    }
    return boundary;
}
function validateMultipartField(fieldName, fieldValue, boundary) {
    ...
    if (CR_OR_LF.test(fieldName)) {
        throw new HttpProxyMiddlewareError(`[HPM] invalid multipart field name "${fieldName}" detected.`, `${ERROR_CODE_PREFIX}_FIELD_NAME`);
    }
    if (CR_OR_LF.test(fieldValue) || fieldValue.includes(boundaryDelimiter)) {
        throw new HttpProxyMiddlewareError(`[HPM] invalid multipart field value for "${fieldName}" detected.`, `${ERROR_CODE_PREFIX}_FIELD_VALUE`);
    }
}
